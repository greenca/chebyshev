% Solve wave equation as a first-order system using Chebyshev collocation and overspecified boundary conditions.
function z = waveEquation1(N)
...
%    B(2*N,2*n+1:2*n+2) = Tn(n,-1)*[0 1];
    ...
    B(2*N-1,2*n+1:2*n+2) = diffTn(n,1)*[0 1];
    
    A(2*N,2*n+1:2*n+2) = [0 0];
    B(2*N,2*n+1:2*n+2) = diffTn(n,-1)*[0 1];
